...
int main(int argc, char *argv[]){
	...
	while (!done) {
		...
		//Single intialization since they are they are the same for both paddles and ball
		float texCoords[] = { 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f, 1.0f, 1.0f, 0.0f, 0.0f, 0.0f };
		...
		if (keys[SDL_SCANCODE_W] && leftPaddle.posY1 < 2.0f){
				leftPaddle.posY1 += 0.001f;
				leftPaddle.posY2 += 0.001f;
		}
		...
		if (keys[SDL_SCANCODE_UP] && rightPaddle.posY1 < 2.0f){
				rightPaddle.posY1 += 0.001f;
				rightPaddle.posY2 += 0.001f;
		}
